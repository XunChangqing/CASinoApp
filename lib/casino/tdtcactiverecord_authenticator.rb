...
class CASino::TdtcActiveRecordAuthenticator
  ...
  def validate(username, password)
    ...
    status = user.send(@options[:status_column])
    #status = user.send(:status)
    Rails.logger.info "******************"
    Rails.logger.info status + 'user.status.normal'
    Rails.logger.info "******************"

    if status == 'user.status.normal' and valid_password?(password, password_from_database)
